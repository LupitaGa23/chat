...
async function enviarMensajeBot() {
    ...
    Estado.chatbotHistorial.push({ role: 'user', content: contenido });
    ...
    const res = await fetchAPI(`${API.chat}?action=chatbot_directo`, {
        ...
        body: JSON.stringify({
            mensaje: contenido,
            historial: Estado.chatbotHistorial.slice(-10) // Últimos 10 mensajes de contexto
        })
    });
    ...
    if (res.error) {
        ...
        Estado.chatbotHistorial.push({ role: 'assistant', content: res.contenido });
